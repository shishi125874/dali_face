...
def recon_view(pnet, rnet, onet, model, image_path, ip, margin=44, image_size=160):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            classess = os.listdir(path_exp)
            video_list = []
            for item in classess:
                folder = os.path.join(path_exp, item)
                if not os.path.isfile(folder):
                    mp4_l = os.listdir(folder)
                    video = os.path.join(folder, mp4_l[0])
                    video_list.append(video)
            if not len(video_list) == 0:
                video_list.sort()
                for i in range(len(video_list)):
                    if os.path.isfile(video_list[i]):
                        begin = datetime.datetime.now()
                        get_video(sess, video_list[i], margin, image_size, images_placeholder, embeddings,
...
def get_video(sess, video_path, margin, image_size, images_placeholder, embeddings, phase_train_placeholder, embding,
    ...
    if os.path.isfile(video_path):
        ...
        send_message(viedoName, faceNames, basees, ip)
...
class EventHandler(ProcessEvent):
    ...
    def process_IN_CLOSE(self, event):
        ...
        tail_name = os.path.basename(images_path)
        _, image_type = os.path.splitext(tail_name)
        if image_type == '.png' or image_type == '.jpg':
            ...
            get_video(self.sess, images_path, self.margin, self.image_size, self.images_placeholder, self.embeddings,
                      self.phase_train_placeholder, self.embding, self.images_label_list, self.pnet, self.rnet,
                      self.onet,
                      self.ip)
            # os.remove(images_path)
...
if __name__ == '__main__':
    ...
    model = sys.path[0]
    model = os.path.join(model, 'models')
    image_path = args.image_path + '/resources/video'
