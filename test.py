...
import Queue  # Queue在3.x中改成了queue
import random
import threading
import time
...
class Producer(threading.Thread):
    ...
    def __init__(self, t_name, queue, path):  # 传入线程名、实例化队列
...
class Consumer(threading.Thread):
    ...
    def run(self):
        for i in range(5):
            val = self.data.get()
            print(
            "%s: %s is consuming. %d in the queue is consumed!" % (time.ctime(), self.getName(), val))  # 编号d队列已经被消费
